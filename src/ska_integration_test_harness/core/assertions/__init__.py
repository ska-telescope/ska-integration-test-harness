...
for defining and executing verifications and synchronisations against a
...
in the past or within a certain time frame.

Representing assertions as classes has not been an easy choice,
...
- The SUT is a complex system made of many components, so sometimes
  you may want to group multiple lower-level checks into a few re-usable
  high-level assertions. Having a class structure helps in organising
  the assertion code into standard modular pieces.
...
- The SUT is a distributed system, so often the assertions are event-based.
...
- :py:class:`~ska_integration_test_harness.core.assertions.TracerAssertion`: a
  ...
  a timeout and of an early stop condition, and provides you with utilities
