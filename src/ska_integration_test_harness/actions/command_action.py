...
class TransientQuiescentCommandAction(TelescopeCommandAction):
    """An action which can be synchronized on quiescent or transient state.

    Some telescope commands can be synchronized both on a quiescent state
    ...
    by setting the ``synchronize_on_transient_state`` attribute.
    """

    def __init__(self):
        super().__init__()

        self.synchronize_on_transient_state = False
        """If True, the action will synchronize on the next transient state
        instead of the next quiescent state. By default, it is False, so
        the action will synchronize on the next quiescent state.
        """

    def set_synchronize_on_transient_state(self, value: bool):
        """Set the synchronize_on_transient_state attribute.

        :param value: If True, the action will synchronize on the next
            transient state instead of the next quiescent state.
            By default, it is False, so the action will synchronize
            on the next quiescent state.
        """
        self.synchronize_on_transient_state = value

    def termination_condition(self) -> list[ExpectedEvent]:
        """Wait for the quiescent or transient state of the subarray.

        By default, the action will synchronize on the next quiescent state,
        but if the synchronize_on_transient_state attribute is True, it will
        synchronize on the next transient state.

        :return: A list of ExpectedEvent objects to wait for.
        """
        if self.synchronize_on_transient_state:
    ...
    def termination_condition_for_transient_state(self) -> list[ExpectedEvent]:
        ...
        when you want to synchronize on the transient state.

        :return: A list of ExpectedEvent objects to wait for, when
            you want to synchronize on the transient state.
        """
    ...
    def termination_condition_for_quiescent_state(self) -> list[ExpectedEvent]:
        ...
        Implement this method to specify the termination condition for
        when you want to synchronize on the quiescent state.

        :return: A list of ExpectedEvent objects to wait for, when
            you want to synchronize on the quiescent state.
        """
